...
class Member(Account):
    ...
    def reserve_book_items(self,book_item):
        None
    
    def renew_total_item(self,book_item):
        None
    
    def incremental_total_books_chekout(self):
        None

    def checkout_book_item(self,book_item):
        if self.get_total_books_checkout()>=Constants.MAX_BOOKS_ISSUED_TO_A_USER:
            print("user has already checkout max books")
            return False
        ...
        if book_reservation!=None and book_reservation.get_member_id()!=self.get_id():
            print("self book is reserverd by another member")

        if not book_item.checout(self.get_id()):
            return False
        
        self.incremental_total_books_chekout()
        return True
